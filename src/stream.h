...
typedef ssize_t (*stream_read_t)(stream_t* s, void* buff, size_t len);
typedef ssize_t (*stream_write_t)(stream_t* s, void* buff, size_t len);
...
static inline ssize_t stream_read(stream_t* s, void* buff, size_t len) {
	return_value_if_fail(s != NULL && s->read != NULL && buff != NULL, 0);
	return s->read(s, buff, len);
}

static inline ssize_t stream_write(stream_t* s, void* buff, size_t len) {
